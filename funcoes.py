...
class Player:
    ...
    def draw(self, screen):
        # Centraliza o personagem na pista considerando sua largura
        screen.blit(self.frames[self.current_frame]), (self.x - (self.width // 2), self.y)
        
        # Desenha os retângulos coloridos apenas quando intangível
        if hasattr(self, 'intangibility') and self.intangibility:
            pygame.draw.rect(screen, (0, 255, 255), (self.x - (self.width // 2), self.y, self.width, self.height), 4)
    ...
    def move(self, direction):
        if direction == "left" and self.lane > 0:
            ...
            self.lane += 1
        # Centraliza o personagem na nova pista
...
class BackgroundManager:
    def __init__(self):
        ...
        # Mantém a proporção original da imagem de fundo
        original_img = self.images[self.current_bg_index]
        original_width, original_height = original_img.get_size()
        ...
        self.current_bg = pygame.transform.scale(original_img, (new_width, new_height))
        self.next_bg = pygame.transform.scale(self.images[self.next_bg_index], (new_width, new_height))
        self.current_y = 0
        self.next_y = -HEIGHT
        self.speed = 5
        self.offset_x = (WIDTH - new_width) // 2  # Centraliza horizontalmente
    ...
    def update(self):
        ...
        if self.current_y >= HEIGHT:
            ...
            self.next_bg_index = self.get_next_random_index(self.current_bg_index)
            original_img = self.images[self.next_bg_index]
            original_width, original_height = original_img.get_size()
            aspect_ratio = original_width / original_height
            new_height = HEIGHT
            new_width = int(new_height * aspect_ratio)
            self.next_bg = pygame.transform.scale(original_img, (new_width, new_height))
            self.next_y = self.current_y - HEIGHT
            
    def draw(self, screen):
        screen.blit(self.current_bg, (self.offset_x, int(self.current_y)))
        screen.blit(self.next_bg, (self.offset_x, int(self.next_y)))
...
def tela_jogo(TELA, contador):
    ...
    while running:
        ...
        player.draw(screen)
        if intangibility:
            pygame.draw.rect(screen, (0, 255, 255), (player.x - (player.width // 2), player.y, player.width, player.height), 4)
        draw_hud(screen, meters, current_obstacle_speed, high_score)
